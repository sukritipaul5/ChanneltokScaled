...
class LFQ(Module):
    ...
    def forward(
        ...
        original_channels = x.shape[1] if x.ndim > 1 else self.dim
        ...
        if channelwise:
            x = rearrange(x, 'b c (h w) -> b c h w', h=spatial_shape[0], w=spatial_shape[1])
        elif is_img_or_video:
            x = rearrange(x, 'b (h w) d -> b d h w', h=spatial_shape[0], w=spatial_shape[1])
            indices = rearrange(indices, 'b (h w) -> b h w', h=spatial_shape[0], w=spatial_shape[1])
...
if __name__ == "__main__":
    # Test non-channelwise mode
    print("Testing non-channelwise mode:")
    model = LFQ(dim=512, codebook_size=1024, num_codebooks=1, keep_num_codebooks_dim=False, quantize_channels=False)
    x = torch.randn(1, 512, 4, 4)
    mask = torch.randn(1,16) > 0
    quantized, indices, aux_loss = model(x,mask=mask)
    print(f"Input shape: {x.shape}")
    print(f"Quantized shape: {quantized.shape}, Indices shape: {indices.shape}, Aux loss: {aux_loss}")

    # Test channelwise mode (dim should match spatial dimensions when using channelwise)
    print("\nTesting channelwise mode:")
    model_cw = LFQ(dim=16, codebook_size=1024, num_codebooks=1, keep_num_codebooks_dim=False, quantize_channels=True)
    x_cw = torch.randn(1, 512, 4, 4)  # 4*4 = 16 spatial dimension
    mask_cw = torch.randn(1,512) > 0
    quantized_cw, indices_cw, aux_loss_cw = model_cw(x_cw,mask=mask_cw)
    print(f"Input shape: {x_cw.shape}")
    print(f"Quantized shape: {quantized_cw.shape}, Indices shape: {indices_cw.shape}, Aux loss: {aux_loss_cw}")
